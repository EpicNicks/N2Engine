...
#pragma warning(pop)
    ...
    public:
        ...
        explicit Vector3(float scalar) : x(scalar), y(scalar), z(scalar), w(0.0f) {}
        ...
        [[nodiscard]] VectorN<float, 3> ToVectorN() const
        {
            return VectorN<float, 3>{x, y, z};
        }
        ...
        [[nodiscard]] float AngleTo(const Vector3 &other) const
        {
            float dot = Dot(other);
        ...
        [[nodiscard]] Vector3 ProjectOnPlane(const Vector3 &planeNormal) const
        {
            const float distance = Dot(planeNormal);
            return *this - planeNormal * distance;
        }

        [[nodiscard]] Vector3 Reject(const Vector3 &onto) const
        {
        ...
        [[nodiscard]] Vector3 Reflect(const Vector3 &normal) const
        {
            ...
            const Vector3 reflection = scalar_mul_func(normal, 2.0f * dot);
        ...
        {
            const Vector3 diff = sub_func(b, a);
            const Vector3 scaled = scalar_mul_func(diff, t);
            return add_func(a, scaled);
        }

        [[nodiscard]] Vector3 Lerp(const Vector3 &other, const float t) const
        {
            return Lerp(*this, other, t);
        }

        [[nodiscard]] Vector3 MoveTowards(const Vector3 &target, const float maxDelta) const
        {
            const Vector3 diff = target - *this;
        ...
        static Vector3 Slerp(const Vector3 &a, const Vector3 &b, float t)
        {
            ...
            const Vector3 relativeVec = (b - a * dot).Normalized();
                                  ...
                                  float smoothTime, float maxSpeed, float deltaTime)
        {
            ...
            if (Vector3 outMinusOrig = output - originalTo; origMinusCurrent.Dot(outMinusOrig) > 0)
            {
                output = originalTo;
        ...
        [[nodiscard]] Vector3 ClampMagnitude(float maxLength) const
        {
        ...
        static Vector3 ClampMagnitude(const Vector3 &v, const float maxLength)
        {
        ...
        [[nodiscard]] bool IsParallel(const Vector3 &other, const float tolerance = 1e-6f) const
        {
            const Vector3 cross = Cross(other);
            return cross.Length() < tolerance;
        }

        [[nodiscard]] bool IsPerpendicular(const Vector3 &other, const float tolerance = 1e-6f) const
        {
            return std::abs(Dot(other)) < tolerance;
        }

        [[nodiscard]] Vector3 GetOrthogonal() const
        {
            if (std::abs(x) < 0.9f)
            {

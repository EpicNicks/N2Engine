...
namespace N2Engine
{
    ...
    private:
        std::unordered_map<Math::UUID, std::shared_ptr<GameObject>, UUIDHash> _gameObjectsByUUID;
        std::unordered_map<Math::UUID, std::shared_ptr<Component>, UUIDHash> _componentsByUUID;
        std::vector<ResolveFunc> _pendingReferences;

    public:
        ReferenceResolver() = default;

        void RegisterGameObject(const Math::UUID &uuid, std::shared_ptr<GameObject> gameObject)
        {
            _gameObjectsByUUID[uuid] = gameObject;
        }

        void RegisterComponent(const Math::UUID &uuid, std::shared_ptr<Component> component)
        {
            _componentsByUUID[uuid] = component;
        }

        std::shared_ptr<GameObject> FindGameObject(const Math::UUID &uuid) const
        {
            auto it = _gameObjectsByUUID.find(uuid);
            return (it != _gameObjectsByUUID.end()) ? it->second : nullptr;
        }

        std::shared_ptr<Component> FindComponent(const Math::UUID &uuid) const
        {
            auto it = _componentsByUUID.find(uuid);
            return (it != _componentsByUUID.end()) ? it->second : nullptr;
        }

        void AddPendingReference(ResolveFunc resolver)
        {
        ...
        size_t GetGameObjectCount() const { return _gameObjectsByUUID.size(); }
        size_t GetComponentCount() const { return _componentsByUUID.size(); }
        size_t GetPendingReferenceCount() const { return _pendingReferences.size(); }
    };
}